...
public class ChaoMungUI extends JFrame {
    ...
    public ChaoMungUI(int idTaiKhoan, String tenNhanVien) {
        ...
        optionCombo.setFont(new Font("Roboto", Font.PLAIN, 14));
        optionCombo.setPreferredSize(new Dimension(150, 25));
        ...
        messengerIcon = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                ImageIcon icon = new ImageIcon("\\E:\\ki4\\java\\QuanLiNhaHang\\src\\image\\messenger.png"); 
                if (icon.getImage() != null && icon.getImage().getWidth(null) > 0) {
                    g2d.drawImage(icon.getImage(), 5, 5, width - 10, height - 10, null);
                } else {
                    g2d.setColor(Color.WHITE);
                    g2d.setFont(new Font("Arial", Font.BOLD, 20));
                    g2d.drawString("M", width / 2 - 5, height / 2 + 5);
                }
            }
        };
        messengerIcon.setPreferredSize(new Dimension(50, 50));
        messengerIcon.setOpaque(false);
        messengerIcon.setBorder(BorderFactory.createEmptyBorder());
        updateMessengerIconPosition(1470, 677);
        messengerIcon.setCursor(new Cursor(Cursor.HAND_CURSOR));

        final int[] dragStartX = {0};
        final int[] dragStartY = {0};
        messengerIcon.addMouseListener(new MouseAdapter() {
            @Override
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 1) {
                    try {
						new ChatUI_nhanvien(idTaiKhoan).setVisible(true);
        ...
        messengerIcon.addMouseMotionListener(new MouseMotionAdapter() {
            @Override
            public void mouseDragged(MouseEvent e) {
                int newX = messengerIcon.getX() + e.getX() - dragStartX[0];
                int newY = messengerIcon.getY() + e.getY() - dragStartY[0];
                newX = Math.max(0, Math.min(newX, getWidth() - messengerIcon.getWidth()));
                newY = Math.max(0, Math.min(newY, getHeight() - messengerIcon.getHeight()));
                messengerIcon.setLocation(newX, newY);
            }
        });

        backgroundPanel.add(messengerIcon);
    ...
    private void updateMessengerIconPosition(int targetX, int targetY) {
        if (messengerIcon != null) {
            int newX = Math.max(0, Math.min(targetX, getWidth() - messengerIcon.getWidth()));
            int newY = Math.max(0, Math.min(targetY, getHeight() - messengerIcon.getHeight()));
            messengerIcon.setLocation(newX, newY);
        }
    }
